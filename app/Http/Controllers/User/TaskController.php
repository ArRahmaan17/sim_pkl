...
class TaskController extends Controller
{
    public function index()
    {
        if (session('auth.role') == 'M') {
            ...
            $pendingTasks = Task::where('status', 'Pending')->whereJsonContains('group', [''.session('auth.group_id')])->count();
            $progressTasks = Task::where('status', 'Progress')->whereJsonContains('group', [''.session('auth.group_id')])->count();
            $endTasks = Task::where('status', 'End')->whereJsonContains('group', [''.session('auth.group_id')])->count();
        }
    ...
    public function store(Request $request)
    {
        if ($request->has('file')) {
            ...
            try {
                ...
                $data['group_id'] = session('auth.group_id');
    ...
    public function activity_update(Request $request)
    {
        if ($request->has('activity_photos')) {
            ...
            if ($last_activity->status == 'Started') {
                $status = 'Analysis';
            } elseif ($last_activity->status == 'Analysis') {
                $status = 'Development';
            }
